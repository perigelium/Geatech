        ...
        for (VisitItem visitItem : visitItems)
        {
            for(ReportStates reportStates : reportStatesList)
            {
                ...
                if (
                        ...
                        && (generalInfoCompletionState == ReportStates.COORDS_SET
                                && photoAddedNumber >= ReportStates.PHOTOS_MIN_ADDED
                                && reportCompletionState == ReportStates.REPORT_COMPLETED)
                        && reportStates.getData_ora_invio_rapporto() == null
                        ) // coords set, photos added, report filled, but has not sent
                {
